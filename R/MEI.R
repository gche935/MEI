...
#  names.lv <- lavaan::lavNames(model.PMI, type = "lv")  # factor name
  no.factor <- length(names.lv)  # number of factors
#  names.ind <- lavaan::lavNames(model.PMI, type = "ov.ind")  # name of indicators
  ...
  # Column 6 - SRMR
    ...
    if (no.items[factor.no] > 2) {
      ...
      for (Model.R in 1: length(Rec.Model)) {
        ...
        PMI.X.1 <- paste0("PSI.X <- '", names.lv[factor.no], " =~ ", "c(", paste0(PSI.load[1, ], collapse=","), ")*", names.ind[FL.kr], " + ","\n")

        for (i in 2: (no.items[factor.no]-1)) {
          PMI.X.1 <- paste0(PMI.X.1, "c(",paste0(PSI.load[i, ], collapse=","),")*", names.ind[FL.kr-1+i], " + ","\n")
        } ## end loop i
        PMI.X.1 <- paste0(PMI.X.1, "c(",paste0(PSI.load[no.items[factor.no], ], collapse=","),")*", names.ind[(FL.kr + no.items[factor.no] - 1)],"\n")
#@      eval(parse(text = PMI.X.1))
...
#@          if (factor.no == 1) {
#@            PSI.X.1 <- paste0(PSI.X.1, "   ", names.ind[i], " ~ c(", paste0(Model.load[i, , Rec.Model[Model.R]], collapse=","), ")*1","\n")
#@          } else {
#@          PSI.X.1 <-
#@           paste0(PSI.X.1, "   ", names.ind[(sum(no.items[1:(factor.no-1)])+i)], " ~ c(", paste0(Model.load[i, , Rec.Model[Model.R]], collapse=","), ")*1","\n")
#@          }
#@        } ## end loop i
...
#@        eval(parse(text = PSI.X.1))
        ...
        for (i in 1: no.items[factor.no]) {
          if (factor.no == 1) {
            PSI.XX[Model.R] <- paste0(PSI.XX[Model.R], "   ", names.ind[i], " ~ c(", paste0(Model.load[i, , Rec.Model[Model.R]], collapse=","), ")*1","\n")
          } else {
            PSI.XX[Model.R] <-
            paste0(PSI.XX[Model.R], "   ", names.ind[(sum(no.items[1:(factor.no-1)])+i)], " ~ c(",paste0(Model.load[i, ,Rec.Model[Model.R]], collapse=","), ")*1","\n")
          }
        } ## end loop i
...
#$    }  ## end (for Model.R)
    ...
    if (no.items[factor.no] < 3) {   ## Begin Models with 2 items ##
      ...
      Recommend.Model <-
        rbind(Recommend.Model, paste0("  ", names.lv[factor.no], " =~ ", "c(", paste0(PSI.load[1,], collapse=","), ")*", names.ind[FL.kr], " + ","\n"))
      Recommend.Model[nrow(Recommend.Model)] <-
          paste0(Recommend.Model[nrow(Recommend.Model)],"  c(",paste0(PSI.load[2, ], collapse=","),")*", names.ind[(FL.kr + no.items[factor.no] - 1)],"\n")
      if (factor.no == 1) {
        Recommend.Model[nrow(Recommend.Model)] <-
          paste0(Recommend.Model[nrow(Recommend.Model)],"   ", names.ind[1], " ~ c(",paste0(R.Model[1, ], collapse=","), ")*1","\n")
        Recommend.Model[nrow(Recommend.Model)] <-
          paste0(Recommend.Model[nrow(Recommend.Model)],"   ", names.ind[2], " ~ c(",paste0(R.Model[2, ], collapse=","), ")*1","\n")
      } else {
        Recommend.Model[nrow(Recommend.Model)] <-
          paste0(Recommend.Model[nrow(Recommend.Model)],"   ", names.ind[(sum(no.items[1:(factor.no-1)])+1)], " ~ c(",paste0(R.Model[1, ], collapse=","), ")*1","\n")
        Recommend.Model[nrow(Recommend.Model)] <-
          paste0(Recommend.Model[nrow(Recommend.Model)],"   ", names.ind[(sum(no.items[1:(factor.no-1)])+2)], " ~ c(",paste0(R.Model[2, ], collapse=","), ")*1","\n")
      } ## end if factor.no == 1
    }  ## End (Models with 2 items)
...
#  names.ind <- lavaan::lavNames(model, type = "ov.ind")  # name of indicators
  ...
  for (i in 1:factor.no) {
    ...
    if (i == 1) {
      for (j in 1:(no.items[i]-1)) { ML.X <- paste0(ML.X, paste0(names.ind[j], " + ")) } ## end loop j
      ML.X <- paste0(ML.X, paste0(names.ind[j+1]),"\n")
    } else {
      for (j in 1:(no.items[i]-1)) { ML.X <- paste0(ML.X, paste0(names.ind[j+sum(no.items[1:(i-1)])], " + ")) } ## end loop j
      ML.X <- paste0(ML.X, paste0(names.ind[j+1+sum(no.items[1:(i-1)])]),"\n")
    }
  } # end loop i
...
## ===== End (Run Configural Model) ===== ##
  ...
  # Find out the number of factors and the number of items per factor #
  parsed <- lavParseModelString(Model.ML.X, as.data.frame = TRUE)
  parsed <- parsed[, c("lhs", "op", "rhs")]
  parsed <- parsed[parsed$op == "=~",]
...
#  names.ind <- lavaan::lavNames(Model.ML.X, type = "ov.ind")  # name of indicators
  temp <- lavaan::parameterEstimates(Model.config)
  no.items <- matrix(1:no.factor, nrow = 1)  # number of items per factor
  for (factor.no in 1:no.factor) {
  ...
  # Column 6 - SRMR
    ...
    if (no.items[factor.no] > 2) {
      ...
      for (Model.R in 1: length(Rec.Model)) {
        ...
        ## == Specify the partial metric invariance models (PMI.X) == ##
        PMI.X.1 <- paste0("PMI.X <- '", "\n", "level: 1", "\n",
                          names.lv[factor.no], " =~ ", paste0(Model.load[1, 1, Rec.Model[Model.R]]), "*", names.ind[(FL.kr+1)], " + ")
        for (i in 2: (no.items[factor.no]-1)) {
          PMI.X.1 <- paste0(PMI.X.1, paste0(Model.load[i, 1, Rec.Model[Model.R]]),"*", names.ind[FL.kr+i], " + ")
        } ## end loop i
        PMI.X.1 <- paste0(PMI.X.1, paste0(Model.load[i+1, 1, Rec.Model[Model.R]]),"*", names.ind[FL.kr+i+1])
        PMI.X.1 <- paste0(PMI.X.1, paste0("\n", "level: 2", "\n",
                          names.lv[no.factor+factor.no], " =~ ", paste0(Model.load[1, 2, Rec.Model[Model.R]]), "*", names.ind[(FL.kr+1)], " + "))
        for (i in 2: (no.items[factor.no]-1)) {
          PMI.X.1 <- paste0(PMI.X.1, paste0(Model.load[i, 2, Rec.Model[Model.R]]),"*", names.ind[FL.kr+i], " + ")
        } ## end loop i
          PMI.X.1 <- paste0(PMI.X.1, paste0(Model.load[i+1, 2, Rec.Model[Model.R]]),"*", names.ind[FL.kr+i+1], " '")
...
#$    }  ## end loop Model.R
    ...
    ## == Save Recommended Model (Recommend.Model) == ##
      ...
      for (i in 2: (no.items[factor.no]-1)) {
        Recommend.Model <- rbind(Recommend.Model, paste0("    ",paste0(R.Model[i,1],"*",names.ind[FL.kr+i], " + ")))
      }  ## end loop i
      Recommend.Model <-
        rbind(Recommend.Model, paste0("    ",paste0(R.Model[no.items[factor.no], 1],"*", names.ind[(FL.kr+no.items[factor.no])])))
      ...
      for (i in 2: (no.items[factor.no]-1)) {
        Recommend.Model <- rbind(Recommend.Model, paste0("    ",paste0(R.Model[i,2],"*",names.ind[FL.kr+i], " + ")))
      }  ## end loop i
      Recommend.Model <-
        rbind(Recommend.Model, paste0("    ",paste0(R.Model[no.items[factor.no], 2],"*", names.ind[(FL.kr+no.items[factor.no])])))
...
#  no.waves <- 3
  ...
  names.lv.model <- unique(parsed[,"lhs"])
  names.ind.model <- unique(parsed[, "rhs"])

#  names.ind.model <- lavaan::lavNames(model, type = "ov")  # item name of input model
...
#  names.ind <- lavaan::lavNames(model.PMI, type = "ov.ind")  # name of indicators
  ...
  ## Extract factor loadings and indicator intercepts ##
  ext <- c(which(temp$op %in% "=~" & temp$rhs %in% names.ind), which(temp$op %in% "~1" & temp$lhs %in% names.ind))
    ...
    if (no.items[factor.no] > 2) {
      ...
      for (Model.R in 1: length(Rec.Model)) {
        ...
        for (j in 1:(no.waves)) {
          if (factor.no == 1) {
            PMI.X.1 <- paste0(PMI.X.1, "  ",
                          names.lv[no.group*(factor.no-1)+j], " =~ ", paste0(PSI.load[1,j]), "*", names.ind[(1+no.items[factor.no]*(j-1))], " + ")
            for (i in 2: (no.items[factor.no]-1)) {
              PMI.X.1 <- paste0(PMI.X.1, PSI.load[i, j],"*", names.ind[(i+no.items[factor.no]*(j-1))], " + ")
            } ## end loop i
            PMI.X.1 <- paste0(PMI.X.1, PSI.load[(i+1), j],"*", names.ind[(i+1+no.items[factor.no]*(j-1))], "\n")
          } else {
            PMI.X.1 <- paste0(PMI.X.1, "  ",
                          names.lv[no.group*(factor.no-1)+j], " =~ ", paste0(PSI.load[1,j]), "*",
                          names.ind[(1+(sum(no.items[1:(factor.no-1)])*no.waves)+no.items[factor.no]*(j-1))], " + ")
            for (i in 2: (no.items[factor.no]-1)) {
              PMI.X.1 <- paste0(PMI.X.1, PSI.load[i, j],"*", names.ind[(i+(sum(no.items[1:(factor.no-1)])*no.waves)+no.items[factor.no]*(j-1))], " + ")
            } ## end loop i
            PMI.X.1 <- paste0(PMI.X.1, PSI.load[(i+1), j],"*", names.ind[(i+1+(sum(no.items[1:(factor.no-1)])*no.waves)+no.items[factor.no]*(j-1))], "\n")
          }
        }
        ...
        for (i in 1:no.items[factor.no]) {
          for (j in 1:(no.waves - 1)) {
            if (factor.no == 1) {
              for (k in (j+1):no.waves) {
                PMI.X.1 <- paste0(PMI.X.1, "  ", names.ind[(no.items[factor.no]*(j-1)+i)], " ~~ ", names.ind[no.items[factor.no]*(k-1)+i], "\n")
              }
            } else {
              for (k in (j+1):no.waves) {
                PMI.X.1 <- paste0(PMI.X.1, "  ", names.ind[(no.items[factor.no]*(j-1)+i + sum(no.items[1:(factor.no-1)])*no.waves)], " ~~ ",
                          names.ind[(no.items[factor.no]*(k-1)+i + sum(no.items[1:(factor.no-1)])*no.waves)],"\n")
        ...
        for (j in 1:(no.waves)) {
          if (factor.no == 1) {
            for (i in 1:no.items[factor.no]) {
              PSI.XX[Model.R] <- paste0(PSI.XX[Model.R], "  ",
                       names.ind[(i+no.items[factor.no]*(j-1))], " ~ ", Model.load[i, j, Rec.Model[Model.R]], "*1", "\n")
            }
          } else {
            for (i in 1:no.items[factor.no]) {
              PSI.XX[Model.R] <- paste0(PSI.XX[Model.R], "  ", names.ind[(i+(sum(no.items[1:(factor.no-1)])*no.waves)+no.items[factor.no]*(j-1))],
      ...
      ## == Save Recommended Model (Recommend.Model) == ##
        ...
        for (j in 1:(no.waves)) {
          if (factor.no == 1) {
            Recommend.Model <- rbind(Recommend.Model, "  ",
                          names.lv[no.group*(factor.no-1)+j], " =~ ", paste0(PSI.load[1,j]), "*", names.ind[(1+no.items[factor.no]*(j-1))], " + ")
            for (i in 2: (no.items[factor.no]-1)) {
              Recommend.Model <- rbind(Recommend.Model, PSI.load[i, j],"*", names.ind[(i+no.items[factor.no]*(j-1))], " + ")
            } ## end loop i
            Recommend.Model <- rbind(Recommend.Model, PSI.load[(i+1), j],"*", names.ind[(i+1+no.items[factor.no]*(j-1))], "\n")
          } else {
            Recommend.Model <- rbind(Recommend.Model, "  ",
                          names.lv[no.group*(factor.no-1)+j], " =~ ", paste0(PSI.load[1,j]), "*",
                          names.ind[(1+(sum(no.items[1:(factor.no-1)])*no.waves)+no.items[factor.no]*(j-1))], " + ")
            for (i in 2: (no.items[factor.no]-1)) {
              Recommend.Model <- rbind(Recommend.Model, PSI.load[i, j],"*", names.ind[(i+(sum(no.items[1:(factor.no-1)])*no.waves)+no.items[factor.no]*(j-1))], " + ")
            } ## end loop i
            Recommend.Model <-
                 rbind(Recommend.Model, PSI.load[(i+1), j],"*", names.ind[(i+1+(sum(no.items[1:(factor.no-1)])*no.waves)+no.items[factor.no]*(j-1))], "\n")
          }
        }
        ...
        for (i in 1:no.items[factor.no]) {
          for (j in 1:(no.waves - 1)) {
            if (factor.no == 1) {
              for (k in (j+1):no.waves) {
                PMI <- paste0("  ", names.ind[(no.items[factor.no]*(j-1)+i)], " ~~ ", names.ind[no.items[factor.no]*(k-1)+i], "\n")
                Recommend.Model <- rbind(Recommend.Model, PMI)
              }
            } else {
              for (k in (j+1):no.waves) {
                PMI <- paste0("  ", names.ind[(no.items[factor.no]*(j-1)+i + sum(no.items[1:(factor.no-1)])*no.waves)], " ~~ ")
                PMI <- paste0(PMI, names.ind[(no.items[factor.no]*(k-1)+i + sum(no.items[1:(factor.no-1)])*no.waves)],"\n")
                Recommend.Model <- rbind(Recommend.Model, PMI)
              }
            }
          }
        ...
        for (j in 1:(no.waves)) {
          if (factor.no == 1) {
            ...
            for (i in 1:no.items[factor.no]) {
              Recommend.Model <- rbind(Recommend.Model, "  ", names.ind[(i+(sum(no.items[1:(factor.no-1)])*no.waves)+no.items[factor.no]*(j-1))],
                                " ~ ", Model.load[i, j, Rec.Model[Model.R]], "*1", "\n")
            } ## end loop i
          }
        }
...
#  source('PSI.txt') ## Run the script
  ...
  ## == Print Factor Loadings == ##
  Final.Model.FL <- matrix(0, sum(no.items), no.waves)
  rownames(Final.Model.FL) <- paste0(" Item ", names.ind.model[1:sum(no.items)])
  colnames(Final.Model.FL) <- c(paste0("Time ", 1:no.group))
  ...
  ## == Print Intercepts == ##
  Final.Model.TX <- matrix(0, sum(no.items), no.group)
  rownames(Final.Model.TX) <- paste0(" Item ", names.ind.model[1:sum(no.items)])
...
PAIRCompareLoadings <- function(model, data.source, Cluster="NULL", Bootstrap=0, Type1 = 0.01, Type1Adj = "PFDR", BMSC="SRMR") {
  ...
  no.lv <- length(names.lv)
  no.ov <- length(names.ind.or)
  for (i in 1: no.lv) { model <- eval(parse(text = (paste0("sub('",names.lv[i],"', '", names.lv[i], "_G1', model, fixed=TRUE)")))) }
  for (i in 1: no.ov) { model <- eval(parse(text = (paste0("sub('",names.ind.or[i],"', '", names.ind.or[i], "_G1', model, fixed=TRUE)")))) }
...
#  names.lv <- lavaan::lavNames(Model.Long, type = "lv")  # factor name
  no.factor <- length(names.lv)/2  # number of factors per wave
#  names.ind <- lavaan::lavNames(Model.Long, type = "ov.ind")  # name of indicators
  no.items.g <- length(names.ind)/2 # number of items per wave
  temp <- lavaan::parameterEstimates(Model.temp.fit)
  no.items <- matrix(1:no.factor, nrow = 1)  # number of items per factor
  for (factor.no in 1:no.factor) { no.items[factor.no] <- sum(temp[,"lhs"] == names.lv[factor.no] & temp[,"op"] == "=~") } ## end loop factor.no
  ...
  for (h in 1:no.factor) {
    for (i in 1:no.items[h]) {
      if (h == 1) {
        Model.Long <- paste0(Model.Long, "  ", names.ind[i], " ~~ ", names.ind[no.items.g+i], "\n")
      } else {
        Model.Long <-
        paste0(Model.Long, "  ", names.ind[+i+sum(no.items[1:(h-1)])], " ~~ ", names.ind[no.items.g+i+sum(no.items[1:(h-1)])],"\n")
      }
    }
  }
